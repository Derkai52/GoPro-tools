...
int download_media_file(CURL *curl, const char *media_file, const char *out_file)
{
    ...
    string path = "http://172.21.112.51:8080/videos/DCIM/" + string(media_file);
    printf("\nDownloading %s to %s\n", media_file, out_file);
...
int main(int argc, char* argv[])
{
    ...
    if(curl != NULL)
    {
        ...
        switch(request)
        {
            case eListFiles:
            {
                const string& path = "http://172.21.112.51:8080/gopro/media/list";
            ...
            {
                string file(argv[2]);
                string path = "http://172.21.112.51:8080/gopro/media/info?path=" + file;
                printf("\n%s info:", file.c_str());
                if(send_write_request(curl, path.c_str(), curl_response, NULL) != ERR_SUCCESS)
                {
                    return ERR_FAILURE;
            ...
            case eHilightFile:
            {
                ...
                const string& path = "http://172.21.112.51:8080/gopro/media/hilight/file?path=" + file + offset_ms;
                if(send_write_request(curl, path.c_str(), curl_response, NULL) != ERR_SUCCESS)
                {
                    return ERR_FAILURE;
                }
            ...
            case eHilightRemove:
            {
                ...
                if(argc >= 4)
                    offset_ms = "&ms=" + string(argv[3]);
                const string& path = "http://172.21.112.51:8080/gopro/media/hilight/remove?path=" + file + offset_ms;
                if(send_write_request(curl, path.c_str(), curl_response, NULL) != ERR_SUCCESS)
                {
                    return ERR_FAILURE;
                }
            ...
            case eHilightMoment:
            {
                const string& path = "http://172.21.112.51:8080/gopro/media/hilight/moment";
                if(send_write_request(curl, path.c_str(), curl_response, NULL) != ERR_SUCCESS)
                {
                    return ERR_FAILURE;
                }
