...
int start_stream(CURL *curl)
{
    ...
    // TODO(Derkai): 这里 172.21.112.51 是当前测试 Gopro 的wifi地址，每个机器不一样，以实际为准。也可以用wireshark进行抓包获取
    curl_easy_setopt(curl, CURLOPT_URL, "http://172.21.112.51:8080/gopro/camera/stream/start");
    // curl_easy_setopt(curl, CURLOPT_URL, "http://172.21.112.51:8080/gopro/camera/stream/start");
...
int stop_stream(CURL *curl)
{
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "http://172.21.112.51:8080/gopro/camera/stream/stop");
    // curl_easy_setopt(curl, CURLOPT_URL, "http://172.21.112.51:8080/gopro/camera/stream/stop");
    if (perform_request(curl) == ERR_FAILURE)
    {
        curl_easy_cleanup(curl);
